...
def fun9_face_morphology():
    ...
    d_img = cv2.dilate(face, mask3)
    cv2.imshow("Dilate", d_img) # 팽창
    cv2.waitKey(0)
    e_img = cv2.erode(face, mask3) # 침식
    cv2.imshow("Erode", e_img)
    cv2.waitKey(0)
    de_img = cv2.erode(d_img, mask3)
    cv2.imshow("D1+ E1", de_img)  # 팽창한거를 침식 close 연산
    cv2.waitKey(0)

    # (4)morphological processing 2
    d2_img = cv2.dilate(d_img, mask3)
    d2e_img = cv2.erode(d2_img, mask3)
    d2e2_img = cv2.erode(d2e_img, mask3)
    cv2.imshow("D2+ E2", d2e2_img)  # close^2
    cv2.waitKey(0)

    #erode(d2e2) -> erode -> dilate -> dilate -> d2e4d2_img
    #    = + 'open^2'
    #코드는 길어서 생략

    # (5)morphological processing 3
    mo_im = cv2.morphologyEx(face, cv2.MORPH_OPEN, mask3, iterations=2)  # open2 = e2d2
